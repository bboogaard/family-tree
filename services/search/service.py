...
class SearchService(ABC):
        ...
        queryset = (
            ...
            .filter(
                Q(
                    ...
                    christian_name__ngrams__score__gte=settings.NAME_NGRAM_MIN_SCORE
                ) | Q (
                    christian_name__alias_for__ngrams__search_query__iexact=search_query,
                    christian_name__alias_for__ngrams__score__gte=settings.NAME_NGRAM_MIN_SCORE
                )
            )
            .annotate(
                name_score=Subquery(
    ...
    def _update(search_query: str):
        ...
        for name in ChristianName.objects.all():
            if name.female_name:
                score_female = NGram.compare(search_query, name.female_name.lower(), N=3)
            else:
                score_female = 0
            if name.male_name:
                score_male = NGram.compare(search_query, name.male_name.lower(), N=3)
            else:
                score_male = 0
